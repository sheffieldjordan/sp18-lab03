...
contract("BadAuctionTest", function(accounts) {
	...
	describe("~BadAuction Works~", function() {
		it("The clean contract should lock on to the auction", async function() {
			...
			assert.equal(target, bad.address, "target locked correctly");
		});
		it("The clean contract should send value to the auction", async function() {
		...
		it(
			"Another clean contract with a lower/the same bid should not " +
				"be able to displace the highest bidder",
			async function() {
				await notPoisoned.bid(args._smallAmount);
				let cleanBalance = await notPoisoned.getBalance.call();
				assert.isBelow(
				...
				let anotherNotPoisoned = await NotPoisoned.new({
					value: args._bigAmount
				});
		...
		it(
			"Another clean contract with a higher bid should be able to " +
				"displace the highest bidder",
			async function() {
				await notPoisoned.bid(args._smallAmount);
				let cleanBalance = await notPoisoned.getBalance.call();
				assert.isBelow(
				...
				assert.equal(
					highestBid.valueOf(),
					args._biggerSmallAmount,
					"new highest bid set"
				);
				assert.equal(
					highestBidder,
					anotherNotPoisoned.address,
	...
	describe("~Push/Pull Attack Vector~", function() {
		...
		it("The poisoned contract should lock on to the auction", async function() {
			let poisonedBalance = await poisoned.getBalance.call();
			assert.equal(
				poisonedBalance.valueOf(),
				args._bigAmount,
				"value set correctly"
			);
		...
		it("The poisoned contract should send value to the auction", async function() {
		...
		it("The bad auction should not be able to accept a new highest bidder", async function() {
			await poisoned.bid(args._smallAmount);
	...
	describe("~Modifiers and Overflow Attack Surface~", function() {
		...
		it("The bad auction should let anybody decrement the standing bid", async function() {
			await notPoisoned.bid(args._smallAmount);
			await poisoned.reduceBid();
			let highestBid = await bad.getHighestBid.call();
			assert.equal(args._smallAmount - 1, highestBid.valueOf());
		});
		it("The bad auction should refund highest bidder when they reduce their bid", async function() {
			await notPoisoned.bid(args._smallAmount);
			await poisoned.reduceBid();
			let notPoisonedBalance = await notPoisoned.getBalance.call();
			assert.equal(
				args._bigAmount - args._smallAmount + 1,
				notPoisonedBalance.valueOf()
			);
		});
		it("The bad auction should let overflow occur", async function() {
			let value = 0;
